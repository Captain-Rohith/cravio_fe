...
export function useCustomerOrderTracking(orderId: string | null) {
  ...
  const [customerMarker, setCustomerMarker] = useState<MapMarker | null>(null);
  const [lastUpdatedLabel, setLastUpdatedLabel] = useState("Not available");
  const animationRef = useRef<number>(0);
  const lastUpdateRef = useRef<number | null>(null);

  const topic = useMemo(
    () => (orderId ? `/topic/orders/${orderId}/customer` : null),
    [orderId],
  );
  ...
  useEffect(() => {
    ...
    client.onConnect = () => {
      ...
      client.subscribe(topic, (message: IMessage) => {
        ...
        const next = { lat: parsed.latitude, lng: parsed.longitude };

        lastUpdateRef.current = Date.now();
        setLastUpdatedLabel("0s ago");
        setLatestEvent({ ...parsed, timestamp: parsed.timestamp ?? new Date().toISOString() });

        setCustomerMarker((current) => {
          const previous = current ?? {
            id: String(parsed.customerId ?? `order-${orderId}-customer`),
